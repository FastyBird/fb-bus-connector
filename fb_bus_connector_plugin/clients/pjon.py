...
"""
FastyBird BUS connector plugin clients module PJON client
"""
...
@inject(alias=BaseClient)
class PjonClient(BaseClient, pjon.ThroughSerialAsync):
    """
    PJON client
    ...
    def handle(self) -> int:
        ...
        try:
            ...
            self._logger.warning("Connection with device was lost")

        except pjon.PJON_Packets_Buffer_Full:
            self._logger.warning("Buffer is full")

        except pjon.PJON_Content_Too_Long:
            self._logger.warning("Content is long")
    ...
    def receive(self, received_payload: bytes, length: int, packet_info: Dict) -> None:
        ...
        if ProtocolVersion.has_value(int(raw_payload[0])) is False:
            self._logger.warning("Received unknown protocol version")

            return

        if Packet.has_value(int(raw_payload[1])) is False:
            self._logger.warning("Received unknown packet")
        ...
        if calculated_crc != in_packet_crc:
            self._logger.warning(f"Invalid CRC check: {calculated_crc} vs {in_packet_crc}")

            return

        if raw_payload[-1] != PacketContent.TERMINATOR.value:
            self._logger.warning("Missing packet terminator")
